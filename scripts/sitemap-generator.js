...
const VUEPRESS_PUBLIC_DIR = join(__dirname, '../docs/.vuepress/public')

// loading site pages definition
const { siteData: { pages } } = requires(resolve('node_modules/@vuepress/core/.temp/internal/siteData'))

//generate metadata for "sitemap" generator
...
if (!existsSync(VUEPRESS_PUBLIC_DIR)) {
  console.error('.vuepress public dir does not exist:', VUEPRESS_PUBLIC_DIR)
  process.exit(1)
}

writeFile(resolve(VUEPRESS_PUBLIC_DIR, 'sitemap.xml'), sitemap.toString(), (err) => {
  if (!err) return console.log(`sitemap generated in: "${VUEPRESS_PUBLIC_DIR}"`)
